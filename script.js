...
function updateCounter() {
  ...
  let hours = now.getHours() - meetDate.getHours();
  let minutes = now.getMinutes() - meetDate.getMinutes();
  let seconds = now.getSeconds() - meetDate.getSeconds();
  let ms = now.getMilliseconds() - meetDate.getMilliseconds();
  ...
  const setText = (id, value, pad) => {
    const el = document.getElementById(id);
    if (el) el.textContent = pad ? String(value).padStart(pad, '0') : String(value);
  };

  setText('cYears', years);
  setText('cMonths', months);
  setText('cWeeks', weeks);
  setText('cDays', days);
  setText('cHours', hours, 2);
  setText('cMinutes', minutes, 2);
  setText('cSeconds', seconds, 2);
  setText('cMs', ms, 3);
}

updateCounter();
setInterval(updateCounter, 31); // ~30 actualizaciones por segundo para que los milisegundos se vean fluidos
...
// Cierra el panel al tocar afuera
...
// Cierra el panel al elegir una sección (opcional, más limpio en móvil)
...
// === Canciones ===
const songs = [
  { title: "One & Only", artist: "Oliver Tree", src: "One-&-Only-Oliver-Tree.mp3" },
  { title: "The Risk", artist: "Laufey", src: "Laufey-The-Risk.wav" },
  { title: "Dream Girl", artist: "Crisaunt", src: "CRISAUNT-DREAM-IRL.wav" },
];
...
// === Intenta sonar apenas carga (funciona si viene de un clic en la bienvenida) ===
function tryAutoplay() {
  ...
  audio.play()
    .then(() => { started = true; })
    .catch(() => {
      // El navegador lo bloqueó: espera la primera interacción como respaldo
...
// === Silenciar / activar sonido (icono en el menú) ===
function toggleMute() {
  muted = !muted;
  audio.muted = muted;
